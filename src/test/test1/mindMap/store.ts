...
import type { MindMapNode, HistorySnapshot, Points, Theme } from "./types";
...
export interface MindMapStore {
  ...
  addKnowledgePoint: (nodeId: string, point: Points) => void;
  updateKnowledgePoint: (
    nodeId: string,
    pointId: string,
    point: Partial<Points>,
  ) => void;
...
const useMindMapStore = create<MindMapStore>((set, get) => {
  return {
    ...
    addKnowledgePoint: (nodeId: string, point: Points) => {
      const state = get();
      const newMindmap = deepCopy(state.mindmap);
      const nodeFound = findNode(newMindmap, nodeId);
      if (nodeFound) {
        if (!nodeFound.points) nodeFound.points = [];
        nodeFound.points.push(point);
        set({ mindmap: newMindmap });
      }
      state.saveSnapshot();
    },
    ...
    updateKnowledgePoint: (
      ...
      const newMindmap = deepCopy(state.mindmap);
      const nodeFound = findNode(newMindmap, nodeId);
      if (nodeFound && nodeFound.points) {
        const pointIndex = nodeFound.points.findIndex((p) => p.id === pointId);
        if (pointIndex >= 0) {
          nodeFound.points[pointIndex] = {
            ...nodeFound.points[pointIndex],
            ...point,
          };
          set({ mindmap: newMindmap });
    ...
    deleteKnowledgePoint: (nodeId: string, pointId: string) => {
      ...
      const nodeFound = findNode(newMindmap, nodeId);
      if (nodeFound && nodeFound.points) {
        nodeFound.points = nodeFound.points.filter((p) => p.id !== pointId);
        set({ mindmap: newMindmap });
      }
      state.saveSnapshot();
    },
