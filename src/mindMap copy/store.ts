import type { HistorySnapshot, KnowledgePoint, MindMapNode, Theme } from './types'
...
import { deepCopy, findNode, setShowChildrenTrue } from './utils/assistFunctions'
import defaultMindmap from './utils/defaultMindmap'

// ===== 辅助函数 =====
function createNewNode(newNodeId: string): MindMapNode {
  return { id: newNodeId, text: NEW_NODE_TEXT, showChildren: true, children: [], knowledgePoints: [] }
}
...
export type MindMapStore = {
  ...
  moveNode: (nodeId: string, targetId: string, parentId: string, isSibling: boolean) => void
  ...
  updateKnowledgePoint: (nodeId: string, pointId: string, point: Partial<KnowledgePoint>) => void
...
const useMindMapStore = create<MindMapStore>((set, get) => ({
  ...
  saveSnapshot: () => {
    ...
    if (state.lastSnapshot) {
      ...
      if (newHistory.length > 0 && newHistory[newHistory.length - 1].mindmap === snapshot.mindmap) {
        const newRedo = [state.lastSnapshot, ...state.redoStack]
        newHistory.pop()
        set({ history: newHistory, redoStack: newRedo, lastSnapshot: snapshot })
        return
      }

      if (state.redoStack.length > 0 && state.redoStack[0].mindmap === snapshot.mindmap) {
  ...
  toggleChildren: (nodeId: string, bool: boolean) => {
    ...
    if (nodeFound && nodeFound.children.length > 0 && nodeFound !== newMindmap) {
  ...
  addChild: (nodeId: string) => {
    ...
    if (nodeFound) {
      ...
      nodeFound.children.push(createNewNode(newNodeId))
      set({ mindmap: newMindmap, curSelect: '', curEdit: newNodeId, curNodeInfo: initialNodeInfo })
    }
    state.saveSnapshot()
  },
  ...
  addSibling: (nodeId: string, parentId: string) => {
    ...
    if (parentId) {
      ...
      if (nodeFound) {
        const insertIndex = nodeFound.children.findIndex(node => node.id === nodeId) + 1
        const newNodeId = uuidv4()
        nodeFound.children.splice(insertIndex, 0, createNewNode(newNodeId))
        set({ mindmap: newMindmap, curSelect: '', curEdit: newNodeId, curNodeInfo: initialNodeInfo })
      }
    }
    state.saveSnapshot()
  ...
  moveNode: (nodeId: string, targetId: string, parentId: string, isSibling: boolean) => {
    const state = get()
    const newMindmap = deepCopy(state.mindmap)
    ...
    if (parent) {
      ...
      if (nodeIndex >= 0) {
        ...
        if (isSibling) {
          const targetIndex = parent.children.findIndex(node => node.id === targetId)
        ...
        set({ mindmap: newMindmap, curSelect: nodeId, curNodeInfo: initialNodeInfo })
      }
    }
    state.saveSnapshot()
  },
  ...
  selectNode: (nodeId: string, selectByClick?: boolean) => {
    ...
    const findNodeInfo = (
      node: MindMapNode,
      searchId: string,
      parent: MindMapNode | { id: string, children: MindMapNode[] } = { id: '', children: [] },
      onLeft = false,
  ...
  deleteNode: (nodeId: string, parentId: string) => {
    ...
    if (parentId) {
      const nodeFound = findNode(newMindmap, parentId)
      if (nodeFound) {
        const deleteIndex = nodeFound.children.findIndex(node => node.id === nodeId)
        if (deleteIndex >= 0) {
          nodeFound.children.splice(deleteIndex, 1)
          set({ mindmap: newMindmap, curSelect: parentId, curNodeInfo: initialNodeInfo })
        }
      }
    }
    state.saveSnapshot()
  ...
  clearNodeStatus: () => {
    set({ curSelect: '', selectByClick: false, curEdit: '', curNodeInfo: initialNodeInfo })
  },
  ...
  undoHistory: () => {
    ...
    if (snapshot) {
      ...
      const newRedo = state.lastSnapshot ? [state.lastSnapshot, ...state.redoStack] : state.redoStack
  ...
  redoHistory: () => {
    ...
    if (snapshot) {
      const newRedo = state.redoStack.slice(1)
      const newHistory = state.lastSnapshot ? [...state.history, state.lastSnapshot] : state.history
      const mindmap = JSON.parse(snapshot.mindmap) as MindMapNode
      set({
        mindmap,
        curSelect: snapshot.cur_node,
  ...
  moveXY: (x: number, y: number) => {
    set(state => ({ x: state.x + x / state.zoom, y: state.y + y / state.zoom }))
  },
  ...
  toggleKnowledgePointModal: (isShow: boolean, nodeId?: string) => {
    set({ knowledgePointModalShow: isShow, knowledgePointNodeId: nodeId || '' })
  },
  ...
  addKnowledgePoint: (nodeId: string, point: KnowledgePoint) => {
    ...
    const nodeFound = findNode(newMindmap, nodeId)
    if (nodeFound) {
      if (!nodeFound.knowledgePoints)
        nodeFound.knowledgePoints = []
  ...
  updateKnowledgePoint: (nodeId: string, pointId: string, point: Partial<KnowledgePoint>) => {
    const state = get()
    const newMindmap = deepCopy(state.mindmap)
    const nodeFound = findNode(newMindmap, nodeId)
    if (nodeFound && nodeFound.knowledgePoints) {
      const pointIndex = nodeFound.knowledgePoints.findIndex(p => p.id === pointId)
      if (pointIndex >= 0) {
        nodeFound.knowledgePoints[pointIndex] = { ...nodeFound.knowledgePoints[pointIndex], ...point }
        set({ mindmap: newMindmap })
      }
    }
    state.saveSnapshot()
  ...
  deleteKnowledgePoint: (nodeId: string, pointId: string) => {
    ...
    const nodeFound = findNode(newMindmap, nodeId)
    if (nodeFound && nodeFound.knowledgePoints) {
      nodeFound.knowledgePoints = nodeFound.knowledgePoints.filter(p => p.id !== pointId)
      set({ mindmap: newMindmap })
    }
    state.saveSnapshot()
  },
  ...
  loadMindmap: (mindmap: MindMapNode) => {
    set({ mindmap, curSelect: mindmap.id, history: [], redoStack: [], lastSnapshot: null })
  },
