...
const Node = ({layer, node, parent, node_refs, on_left}) => {
    const self = useRef();
    const isRightClick = useRef(false); // 标记右键触发的选中，避免 scrollIntoView 干扰菜单位置
    ...
    const handleContextMenu = useCallback((e) => {
        ...
        e.stopPropagation();
        mindmapHook.selectNode(node.id, true);
        // 等一帧让 scrollIntoView 完成后再获取节点位置，避免画布移动导致菜单位置偏移
